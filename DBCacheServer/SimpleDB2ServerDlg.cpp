...
// String g_DbmsConnectionString = L"Provider=MSDASQL.1;Driver={MySQL ODBC 5.3 UNICODE Driver};Persist Security Info=True;Data Source=MySQL_Unicode;Server=127.0.0.1;port=3306;Initial Catalog=prouddb2-test;User ID=testserver;Password=yourpasswordhere;Database=ProudDB2-Test;AllowUserVariables=True;";

String dbmsStringForFormat = L"Provider=MSDASQL.1;Driver={MySQL ODBC 5.3 UNICODE Driver};Persist Security Info=True;Data Source=MySQL_Unicode;Server=%s;port=%d;Initial Catalog=%s;User ID=%s;Password=%s;Database=%s;AllowUserVariables=True;";

//////////////////////////////////////////////////////////////////////////
//DBMS String을 생성한다.
//////////////////////////////////////////////////////////////////////////
static String CreateDBMSString(String server_ip, int port, String uid, String pw, String DataBaseName)
{
	LPWSTR ws = (wchar_t*)dbmsStringForFormat.GetString();
	String outval;
	outval.Format(ws, server_ip, port, DataBaseName, uid, pw, DataBaseName);
	return outval;
}
...
BOOL CSimpleDB2ServerDlg::OnInitDialog()
{
	...
	String errMsg;
	if(!TrialStartDbServer(errMsg))
	{
		AfxMessageBox(errMsg,MB_ICONHAND|MB_OK);
		PostQuitMessage(0);
	}
	else
	{
...
bool CSimpleDB2ServerDlg::TrialStartDbServer(String &outErrMsg )
{

	String url = L"dbserver.myurl.com";
	int port = 3306;
	String userID = L"testserver";
	String pw = L"yourpassword";
	String db = L"database_name";

	CDbCacheServer2StartParameter p1;
	p1.m_authenticationKey = g_DbAuthKey;
	p1.m_DbmsConnectionString = CreateDBMSString(url, port, userID, pw, db);
	p1.m_delegate = this;
	...
	//DB2Cache sample 과 동일하게 root 테이블
	use_users.m_rootTableName = L"use_users";

	//DB2Cache sample 과 동일하게 child 테이블
	use_users.m_childTableNames.Add(L"use_attendance");
	use_users.m_childTableNames.Add(L"use_bugreport");
	use_users.m_childTableNames.Add(L"use_characters");
	use_users.m_childTableNames.Add(L"use_chest_drop_counter");
	use_users.m_childTableNames.Add(L"use_configurations");
	use_users.m_childTableNames.Add(L"use_couponhistory");
	use_users.m_childTableNames.Add(L"use_coupons");
	use_users.m_childTableNames.Add(L"use_dailyquest");
	use_users.m_childTableNames.Add(L"use_deck");
	use_users.m_childTableNames.Add(L"use_deckset");
	use_users.m_childTableNames.Add(L"use_goods");
	use_users.m_childTableNames.Add(L"use_logged_in");
	use_users.m_childTableNames.Add(L"use_membership");
	use_users.m_childTableNames.Add(L"use_progress");
	use_users.m_childTableNames.Add(L"use_recipes");
	use_users.m_childTableNames.Add(L"use_singleplay");
	use_users.m_childTableNames.Add(L"use_singleplay_log");
	use_users.m_childTableNames.Add(L"use_skills");
	use_users.m_childTableNames.Add(L"use_team");
	use_users.m_childTableNames.Add(L"use_title");

	//물론 모든 테이블은 UUID 등의 설정이 끝나있어야 한다.

	p1.m_tableNames.Add(use_users);
	//이런식으로 여러개 추가
	//p1.m_tableNames.Add(use_duel);
...
{
	String txt;
	txt.Format(L"%s occured at %s", (LPCWSTR)errorInfo->ToString(), __FUNCTIONW__);
	LogEvent(txt);
}
